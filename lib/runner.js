...
function Runner(strategy, summarize) {
  ...
  if (strategy.summarizers == null) {
    strategy.summarizers = {"overview": []};
  }

  if (this.summarize == false) {
  ...
  this.summarizers = this.resolve_dependency("summarizers");

  // This error handling sucsk, but apparently I need it.
  this.feeders.on("error", this.error.bind(this));
  this.analyzers.on("error", this.error.bind(this));
  this.deciders.on("error", this.error.bind(this));
  this.executors.on("error", this.error.bind(this));
  this.summarizers.on("error", this.error.bind(this));
...
Runner.prototype.run = function() {
  ...
  // Feeder events
  this.bubble("tick", this.feeders, [this.summarizers, this.executors, this.analyzers, this.deciders, this]);
  this.bubble("book", this.feeders, [this.summarizers, this.executors, this.analyzers, this.deciders]);
  ...
  // Decider events
  this.bubble("margin_buy", this.deciders, [this.summarizers, this.executors]);
  this.bubble("margin_sell", this.deciders, [this.summarizers, this.executors]);

  // Executor events
  this.bubble("supply_change", this.executors, [this.summarizers, this.deciders]);

  this.feeders.event("set_state", this.state);
  this.deciders.event("set_state", this.state);
  this.executors.event("set_state", this.state);
  this.summarizers.event("set_state", this.state);
...
Runner.prototype.stop = function(err) {
  ...
  if (!err) {
    ...
    this.summarizers.event("summarize", summary);
  }
...
Runner.prototype.error = function(err) {
  console.log(err.stack || e.message || e);
  process.exit();
};
