...
function Position(type, opened_by, state) {
...
Position.prototype.open = function(amount, price) {
  ...
  this.opening_trade = new Trade(this.type, amount, price, this.state);

  this.status = "pending_open";
...
Position.prototype.close = function(price, decider, timestamp) {
  var self = this;
  ...
  this.closing_trade = new Trade(type, this.opening_trade.executed_amount, price, this.state);

  this.status = "pending_close";
